classdef BACKWARD_SOLVER_RYTOV_precompute < BACKWARD_SOLVER
    ...
    methods
        function h=BACKWARD_SOLVER_RYTOV_precompute(params)
            h@BACKWARD_SOLVER(params);
        end
        function [RI]=pre_solve(h,bg,sp,retPhase)
            warning('off','all');
            ...
            if size(bg,1)~=size(bg,2)
                error('Please input a square field');
            end
            if ~isequal(size(bg),size(sp))
            ...
            if h.parameters.size(1)~=size(bg,1) || h.parameters.size(2)~=size(bg,2)
...
%             end
            ...
            for kk= 1 :thetaSize
                ...
                Kx=double(round(Kx/h.utility.fourier_space.res{2}+floor(h.parameters.size(2)/2)+1)); 
                Ky=double(round(Ky/h.utility.fourier_space.res{1}+floor(h.parameters.size(1)/2)+1)); 
                Kz=double(round(Kz/h.utility.fourier_space.res{3}+floor(h.parameters.size(3)/2)+1));
                Kzp=(Kz-1)*h.parameters.size(1).*h.parameters.size(2)+(Kx-1)*h.parameters.size(1)+Ky;
                %Kzp=double((Kx-1)*h.parameters.size(1)+Ky);
                %Kzp=Kzp+(Kz-1)*double(h.parameters.size(1).*h.parameters.size(2));
            ...
            h.support=gather(Count>0);
            h.count=gather(Count);
