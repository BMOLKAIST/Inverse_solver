classdef BACKWARD_SOLVER_MULTI_FAST < BACKWARD_SOLVER
    ...
    methods(Static)
        function params=get_default_parameters(init_params)
            ...
            params.tv_param=0.005;%0.001;%0.1;
    ...
    methods
        function [gradient_RI,err]=get_gradiant_static(h,RI,input_field,output_field,forward_solver)
            warning ('off','all');
            ...
            if h.parameters.filter_by_count
                ...
                diff_field=diff_field.*reshape(h.filter,size(h.filter,1),size(h.filter,2),1,[]).*size(diff_field,4)/3;
        ...
        function h=BACKWARD_SOLVER_MULTI_FAST(params,init_solver)
        ...
        function [RI]=solve(h,input_field,output_field)
            ...
            error('also add a filter in the TV');
