function out_mat=TV_FISTA_inner_v2(in_mat,lambda,Vmin, Vmax, Vimag_max, non_neg,dirichlet_boundary,inner_itt,use_gpu)
...
if Vimag_max == 0
    ...
    for mm=1:inner_itt
        ...
        P_np=TV_L_trans(project_non_neg(in_mat-lambda*TV_L(R),Vmin, Vmax, Vimag_max,non_neg,dirichlet_boundary));
        for kk=1:dim_num
    ...
    out_mat=project_non_neg(in_mat-lambda*TV_L(P_np),Vmin, Vmax, Vimag_max,non_neg,dirichlet_boundary);
    
end

%% helping functions

function in_mat=project_non_neg(in_mat,Vmin, Vmax, Vimag_max,non_neg,dirichlet_boundary)
if non_neg
    in_mat(real(in_mat)<Vmin)=Vmin + 1i*imag(in_mat(real(in_mat)<Vmin));
    in_mat(real(in_mat)>Vmax)=Vmax+ 1i*imag(in_mat(real(in_mat)>Vmax));
end
in_mat(imag(in_mat)<0)=real(in_mat(imag(in_mat)<0));
if Vimag_max >0
    in_mat(imag(in_mat)>Vimag_max)=real(in_mat(imag(in_mat)>Vimag_max)) + 1i.*Vimag_max;
end
