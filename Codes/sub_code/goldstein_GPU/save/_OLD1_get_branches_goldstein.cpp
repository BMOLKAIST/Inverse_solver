	...
	int nrhs, mxArray const *prhs[])
{
	...
	char const * const errMsg = "Provide all input as int32 to MEX file.";
	...
	if (dims_shortest_list!=3 || dim_array_shortest_list[0]!=2 ) {
		mexErrMsgIdAndTxt(errId, errMsg2);
	}

	if (dim_array_residues[1]!=dim_array_shortest_list[2]) {
		mexErrMsgIdAndTxt(errId, errMsg2);
	}
	...
	//get the branches
	std::vector<branche_coordinates> returned_branches = get_branches_goldstein_compute( presidues, pshortest_list, psquare_size,  plookup_table, plookup_z_start, residue_number, z_stack_number, lookup_length, precomputed_distance_number, size_residue_3D);
	...
	for (int i = 0; i < returned_branches.size(); i++) {//fill up the output array 
		...
		poutput[i*column_number+4] = returned_branches[i].jump_value;
	}
